<?php
	...
	while($row = mysqli_fetch_assoc($result))
	{
		...
		<div class="form-inline" id="edit-data">
			...
			<div class="form-group col-md-3">
				<input type="text" name="mobile" id="mobile" placeholder="Mobile" class="form-control" value="<?php echo $row['username']; ?>" required/>
			</div>
			<div class="form-group col-md-3">
				<input type="text" id="password" name="password" placeholder="Password" class="form-control" value="<?php echo $row['password']; ?>" required />
			</div>
			<div class="form-group col-md-3">
			<button type="button" class="btn btn-primary update" id="<?php echo $row['user_id']; ?>" name="update">Update Record</button>
